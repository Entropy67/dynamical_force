...
Bonds informaiton class
    ...
    Log:
        ...
        07/04/2019:  redefined the rebinding rate, remove e2 and x2
"""
...
def getBonds(e1=10, e2=10, x1=1.5, x2=2.0, output=True, ton=0, pot="cusp"):
...
class Bond:
    ...
    def setup(self, update_rate=True):
        ...
        if self.potential == "cusp":
            ...
            if update_rate:
                ...
                self.t_off = kramer(self.e1, self.k1, self.f, self.m)
            ...
            self.x_break = 1.2*self.x1
            #self.x_break = 0.5*self.xb*(1+np.sqrt(1-self.f/self.f_drift))

        elif self.potential == "phenomenon":
            self.t_off = kramer0(self.e1, self.x1, self.f, self.m)
            self.koff = 1.0/self.t_off
            if self.frozen:
                self.koff=0
    ...
    def set_binding(self, t_on):

        self.t_on = t_on

        self.kon = 1.0/t_on

        #print("setting rebinding timescale:{0:.3f} ".format( self.t_on))
    ...
    def get_off_rate(self, f=None):
        ...
        if self.frozen:
            return 0
        if self.potential=="cusp":
            return 1.0/kramer(self.e1, self.k1, f, self.m)
        elif self.potential=="phenomenon":
            return 1.0/kramer0(self.e1, self.x1, f, self.m)
        else:
            return 0
    ...
    def info(self):
        fs = np.sqrt(2.0*self.e1*self.k1*kT)
        ...
        print("force, f0={0:.3f}, Eb={1:.3f}".format(self.f0, self.e1*(1-self.f/fs)))
...
def kramer0(e, xb, f, m=1.0):
    ## f in N
    tau0 = 100
    dE = e - f*xb*1.0E-9/kT
    return tau0*np.exp(dE)
